...
int getFontPlace(const string& line);

int main(int argc, char* argv[]) {
    int start = -1, end = -1, i, j, place, _place = 0, oldFontLength = 0, oldFontStart = 0, sizeB4Font = 0;
    ...
    if (start == -1 || end == -1) {
        cout << "Could not find the necessary sections in the file." << endl;
        return 1;
    }
    ...
    cout << "Writing output to " << filename << " ...\n";
...
int getFontPlace(const string& line) {
    ...
    for (i = 0; i < line.length(); i++) {
        if (line[i] == ',') pos++;
        if (line.substr(i, 8) == "Fontname") break;
    }
    return pos;
}
